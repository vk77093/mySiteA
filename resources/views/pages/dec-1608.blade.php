@extends('includesFiles.main')
@section('mainBody')
<div id="carouselExampleControls" class="carsoule slide" data-ride="carousel"style="margin-top: 50px">
<div class="carousel-inner">
 <div class="carousel-item active">

     <img src="{{asset('assets/images/WhiteChocolates/White-1608Header.jpg')}}" class="d-block w-100" alt="aariafoods white chocolate header">
    </div>
</div>
</div>
<div class="container mt-2 mb-2">
    <div data-aos="fade-right" data-aos-offset="300" data-aos-easing="ease-in-sine">
        <div class="mr-5 ml-5 mt-4">
    <h6 class="about-us"> 1608-WHITE CHOCOLATE COUVERTURE </h6>
</div>
    </div>
<div class="row product-top">
    <div class="col-sm-6 col-md-6 text-center product-top">
<p class="comp">Composition</p>
<span class="comp">INGREDIENTS:</span> <span class="compDesc">Sugar, Cocoa Butter, Whole Milk Powder, Cream Powder, Emulsifier (INS322)

Contains Added Natural Flavour (Vanilla Extract)</span><br>
<span class="compDesc">Contains Added Natural Flavor (Vanilla Extract)</span> <br>
<span class="comp ml-2">Allergen: </span>
<span class="compDesc">Soy Lecithin, May Contains Traces

                   of Nuts and Milk Solids.</span><br>
                   <span class="comp">*The Product Doesn’t Contain Gluten*</span>
                   <section class="mt-3">
                      <a href="/white-chocolates/dec-1608/technical-specification-1608"><button class="btn btn-myInfo btn-lg">Click For technical specs..</button></a>
                   </section>
    </div>
    <div class="col-sm-6 col-md-6 text-center mb-1 mt-1">
<table class="table tableCol">
<tr>
    <td colspan="2" class="comp">NUTRITIONAL  INFORMATION</td>
  </tr>
  <tr>
    <td colspan="2" class="comp">Average Quantity Per 100g*</td>
  </tr>
  <tr>
      <td class="compDesc">Energy (kcal)</td>
      <td class="compDesc">565.00</td>
  </tr>
  <tr>
      <td class="compDesc">Total Fat (g)</td>
       <td class="compDesc">35.00</td>
  </tr>
  <tr>
      <td class="compDesc">Saturated Fat (g)</td>
       <td class="compDesc">22.00</td>
  </tr>
  <tr>
      <td class="compDesc">Carbohydrates (g)</td>
       <td class="compDesc">57.70</td>
  </tr>
  <tr>
      <td class="compDesc">Sugar (g)</td>
       <td class="compDesc">57.70</td>
  </tr>
  <tr>
      <td class="compDesc">Protein (g)</td>
       <td class="compDesc">4.00</td>
  </tr>
  <tr>
      <td class="compDesc">Dietary Fiber (g)</td>
       <td class="compDesc">0.00</td>
  </tr>
  <tr>
      <td class="compDesc">Sodium (mg)</td>
       <td class="compDesc">80.00</td>
  </tr>
</table>
    </div>
</div>
</div>
<div class="container mt-2 mb-2">
    <div data-aos="fade-right" data-aos-offset="300" data-aos-easing="ease-in-sine">
    <div class="mr-5 ml-5 mt-4">
    <h6 class="about-us"> DISCOVER WHITE CHOCOLATE AND MANGO MINT GANACHE </h6>
    </div>
    </div>
        <div class="row product-top">
<div class="col-sm-7 col-md-7 text-center">
<img src="{{asset('assets/images/WhiteChocolates/1608-Recipe-1024x683.jpg')}}" class="img-fluid hvr-float-shadow lazy" loading="lazy" alt="WHITE CHOCOLATE AND MANGO MINT GANACHE recipe by aariafoods">
</div>
<div class="col-sm-5 col-md-5 text-center">
<table class="table tableCol">
<tr>
    <td class="comp">S.No.</td>
    <td class="comp">Ingredients</td>
    <td class="comp">Quantity</td>
</tr>
<tr>
    <td class="compDesc">1</td>
    <td class="compDesc">Aaria White Chocolate</td>
    <td class="compDesc">1300 g</td>
</tr>
<tr>
    <td class="compDesc">2</td>
    <td class="compDesc">Full Fat Creame 35%</td>
    <td class="compDesc">200 g</td>
</tr>
<tr>
    <td class="compDesc">3</td>
    <td class="compDesc">Fresh Mango Puree</td>
    <td class="compDesc">300 g</td>
</tr>
<tr>
    <td class="compDesc">4</td>
    <td class="compDesc">Unsalted Butter</td>
    <td class="compDesc">200 g</td>
</tr>
<tr>
    <td class="compDesc">5</td>
    <td class="compDesc">Liquid Glucose</td>
    <td class="compDesc">75 g</td>
</tr>
<tr>
    <td class="compDesc">6</td>
    <td class="compDesc">Mint Extract</td>
    <td class="compDesc">5 ml</td>
</tr>


</table>
</div>
    </div>
<div class="row product-top">

    <div class="col-sm-12 col-md-12">
        <p class="comp">PREPARATION</p>
<ul>
    <li class="compDesc">Heat cream and mango puree together and simmer.</li>
    <li class="compDesc">Add liquid glucose and bring it to boil. After a single boil bring it down and add Aaria White chocolate to the warm mixture.</li>
    <li class="compDesc">Add butter and mint , allow it to rest for 2 minutes.</li>
    <li class="compDesc">Pour the same in a 1/2 inch height lined tray and set aside.</li>
    <li class="compDesc">Allow it to set overnight Once set, cut it in 1 square inch cube and enrobe the same in Aaria white chocolate.</li>
</ul>
    </div>
</div>
</div>
<div class="container mt-2">
<div data-aos="fade-right" data-aos-duration="3000" data-aos-easing="ease-in-sine">
<div class="text-center mr-5 ml-5">
    <h6 class="about-us">EXPLORE OUR RANGE</h6>
</div>
</div>
<div class="container w-50 mt-5">
    <div class="row justify-content-md-center">
<div class="col-sm-4">
    <div class="img hvr-float-shadow"><a href="/dark-chocolates/dec-1442"><img src="{{asset('assets/images/DarkChocolates/1506Bot.jpg')}}" class="img-fluid center-block lazy" loading="lazy" alt="Responsive image"></a></div>
    <p class="text-center recip mt-2">RECIPE NO° 1506</p>
</div>
<div class="col-sm-4">
    <div class="img hvr-float-shadow"><a href="/dark-chocolates/dec-1401"><img src="{{asset('assets/images/DarkChocolates/1401.jpg')}}" class="img-fluid center-block lazy" loading="lazy" alt="Responsive image"></a></div>
    <p class="text-center recip mt-2">RECIPE NO° 1401</p>
</div>
<div class="col-sm-4">
<div class="img hvr-float-shadow"><a href="/dark-chocolates/dec-1388"><img src="{{asset('assets/images/DarkChocolates/1388Bot.jpg')}}" class="img-fluid center-block lazy" loading="lazy" alt="Responsive image"></a></div>
<p class="text-center recip mt-2">RECIPE NO° 1388</p>
</div>
    </div>
</div>
</div>
@endsection
